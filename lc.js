...
let learningData = {
  hu: {
    ...
    // THÊM MỚI: alternatingFails để theo dõi số lần đảo sai
    smartMode: { active: 'alternating', lossesInARow: 0, alternatingFails: 0, lastPred: null }, 
    adaptiveThresholds: {},
    recentAccuracy: []
  },
  md5: {
...
function loadLearningData() {
  try {
    if (fs.existsSync(LEARNING_FILE)) {
      ...
      // Đảm bảo khởi tạo các object mới nếu file cũ chưa có
      if (!learningData.hu.smartMode) learningData.hu.smartMode = { active: 'alternating', lossesInARow: 0, alternatingFails: 0, lastPred: null };
      if (learningData.hu.smartMode.alternatingFails === undefined) learningData.hu.smartMode.alternatingFails = 0;
      
      if (!learningData.md5.smartMode) learningData.md5.smartMode = { active: 'alternating', lossesInARow: 0, alternatingFails: 0, lastPred: null };
      if (learningData.md5.smartMode.alternatingFails === undefined) learningData.md5.smartMode.alternatingFails = 0;
...
async function updateHistoryStatus(type) {
  try {
    ...
    for (const record of predictionHistory[type]) {
      if (record.ket_qua_du_doan && record.ket_qua_du_doan !== '' && record.Lich_su_kieu_moi) continue;
      ...
      if (actualResult) {
        ...
        // KIỂU HIỂN THỊ MỚI ĐÚNG CHUẨN VIP: Phiên #6910406: 🔵 XỈU → ✅ THẮNG
        const colorIcon = ketQuaThucTe === 'Tài' ? '🔴 TÀI' : '🔵 XỈU';
        const winIcon = record.ket_qua_du_doan === 'Đúng ✅' ? '✅ THẮNG' : '❌ THUA';
        record.Lich_su_kieu_moi = `Phiên #${record.Phien_hien_tai}: ${colorIcon} → ${winIcon}`;
  ...
  for (const pred of learningData[type].predictions) {
    ...
    if (actualResult) {
      ...
      if (pred.isCorrect) {
        ...
        // THUẬT TOÁN MỚI: Thắng -> Reset chuỗi thua, reset lỗi đảo, duy trì đảo
        learningData[type].smartMode.lossesInARow = 0;
        learningData[type].smartMode.alternatingFails = 0;
        if (learningData[type].smartMode.active === 'super_learning' || learningData[type].smartMode.active === 'normal') {
          // Thoát chuỗi đen quay lại đảo thông minh
          learningData[type].smartMode.active = 'alternating';
        }
        ...
        // THUẬT TOÁN MỚI: Đếm số tay thua
        learningData[type].smartMode.lossesInARow++;
        
        // Logic Đảo Sai: Nếu đang ở chế độ đảo mà sai -> Tăng biến đếm sai
        if (learningData[type].smartMode.active === 'alternating') {
            learningData[type].smartMode.alternatingFails++;
            // Đảo sai quá 3 lần -> Tắt đảo, quay về phân tích chuẩn ban đầu
            if (learningData[type].smartMode.alternatingFails >= 3) {
                learningData[type].smartMode.active = 'normal';
                learningData[type].smartMode.alternatingFails = 0;
                learningData[type].smartMode.lossesInARow = 0; // Reset để không bị nhầm vào siêu tốc ngay
            }
        }

        // Thua quá 2 tay liên tiếp (tính chung) -> Bật Học siêu tốc
        if (learningData[type].smartMode.lossesInARow >= 2) {
            learningData[type].smartMode.active = 'super_learning'; // Kích hoạt học siêu tốc
            learningData[type].smartMode.lossesInARow = 0; // Reset chờ đợt kiểm tra mới
        }
      }
...
function calculateAdvancedPrediction(data, type) {
  ...
  // =========================================================================
  // === BẮT ĐẦU: LOGIC SIÊU CẤP MỚI DÀNH CHO SMART MODE VÀ BÁM BỆT DÀI ===
  // =========================================================================
  const lastResult1 = results[0];
  const lastResult2 = results[1];
  const lastResult3 = results[2];

  // Phát hiện bệt từ ngắn (2 tay) trở lên
  const isBet = lastResult1 === lastResult2 && lastResult1 !== undefined; 
  const isBet3 = lastResult1 === lastResult2 && lastResult2 === lastResult3 && lastResult1 !== undefined; 

  if (isBet) {
      // TUYỆT ĐỐI KHÔNG ĐẢO KHI CÓ BỆT - Ưu tiên bắt đúng từ ngắn lên dài
      finalPrediction = lastResult1;
      finalConfidence = Math.min(95, finalConfidence + 12); // Củng cố tự tin theo bệt mạnh hơn
      factors.unshift(`🔥 BÁM BỆT VIP: Phát hiện Bệt ${isBet3 ? 'dài' : 'ngắn'} -> Quyết đoán bám ${finalPrediction}`);
  } else {
      if (learningData[type].smartMode.active === 'alternating') {
          // CHẾ ĐỘ ĐẢO THÔNG MINH - CHỈ KÍCH HOẠT KHI KHÔNG BỆT VÀ ĐANG THẮNG
          if (streakInfo.currentStreak > 0 && learningData[type].smartMode.lastPred) {
              finalPrediction = learningData[type].smartMode.lastPred === 'Tài' ? 'Xỉu' : 'Tài';
              factors.unshift(`🔄 ĐẢO THÔNG MINH: Thắng 1 tay đảo 1 tay -> Bắt ${finalPrediction}`);
          }
      } else if (learningData[type].smartMode.active === 'super_learning') {
          // CHẾ ĐỘ HỌC SIÊU TỐC - KHI THUA QUÁ 2 TAY
          // Bắt cực nhạy theo kết quả mới nhất để cắt dây xui
          finalConfidence = Math.min(98, finalConfidence + 15);
          
          // Phân tích sát 3 phiên gần nhất - nếu lệch sẽ bẻ thẳng
          const recent3 = results.slice(0, 3);
          const taiCount3 = recent3.filter(r => r === 'Tài').length;
          
          if (taiCount3 >= 2) finalPrediction = 'Tài';
          if (taiCount3 <= 1) finalPrediction = 'Xỉu';

          // Nếu chênh lệch quá xa thì tin tưởng siêu phân tích điểm số
          if (taiScore > xiuScore * 1.3) finalPrediction = 'Tài';
          if (xiuScore > taiScore * 1.3) finalPrediction = 'Xỉu';

          factors.unshift(`⚡ HỌC SIÊU TỐC & SIÊU PHÂN TÍCH: Tự động sửa sai -> Chốt ${finalPrediction}`);
      } else if (learningData[type].smartMode.active === 'normal') {
          // MỚI: QUAY VỀ PHÂN TÍCH CHUẨN NẾU ĐẢO SAI QUÁ 3 LẦN
          factors.unshift(`🧠 PHÂN TÍCH CƠ BẢN: Đảo sai 3 lần, quay về phân tích chuẩn -> Bắt ${finalPrediction}`);
      }
  }

  // Cập nhật lại tay dự đoán cuối cùng cho thuật toán lần sau
  learningData[type].smartMode.lastPred = finalPrediction;
  // =========================================================================
  // === KẾT THÚC: LOGIC SIÊU CẤP MỚI ===
  // =========================================================================
  ...
  return {
    ...
    detailedAnalysis: {
      ...
      learningStats: {
        ...
        activeMode: learningData[type].smartMode.active, // Hiển thị chế độ hiện tại
        alternatingFails: learningData[type].smartMode.alternatingFails // Hiện số lần đảo sai
      }
    }
  };
}
...
function savePredictionToHistory(type, phien, prediction, confidence, latestData) {
  const record = {
    ...
    Du_doan: prediction,
    ket_qua_du_doan: '',
    Lich_su_kieu_moi: '', // Thêm sẵn field mới
    id: '@tiendataox',
    timestamp: new Date().toISOString()
  };
...
// XOÁ GIAO DIỆN LUNG TUNG - LÀM SẠCH LỊCH SỬ CỰC VIP
app.get('/lc79-hu/lichsu', async (req, res) => {
  try {
    await updateHistoryStatus('hu');
    
    // Chỉ lấy mảng các chuỗi đẹp, sạch sẽ bỏ hết mấy object lung tung
    const cleanHistory = predictionHistory.hu
      .filter(r => r.Lich_su_kieu_moi && (r.Lich_su_kieu_moi.includes('✅') || r.Lich_su_kieu_moi.includes('❌')))
      .map(r => r.Lich_su_kieu_moi);
    
    res.json({
      type: 'Lẩu Cua 79 - Tài Xỉu Hũ',
      total_da_co_ket_qua: cleanHistory.length,
      lich_su_chi_tiet: cleanHistory // Rất sạch và chuyên nghiệp
    });
  } catch (error) {
    res.json({
      type: 'Lẩu Cua 79 - Tài Xỉu Hũ',
      error: 'Lỗi xử lý lịch sử'
    });
  }
});
...
app.get('/lc79-md5/lichsu', async (req, res) => {
  try {
    ...
    const cleanHistory = predictionHistory.md5
      .filter(r => r.Lich_su_kieu_moi && (r.Lich_su_kieu_moi.includes('✅') || r.Lich_su_kieu_moi.includes('❌')))
      .map(r => r.Lich_su_kieu_moi);
...
app.get('/lc79-hu/learning', (req, res) => {
  ...
  res.json({
    ...
    smartModeState: stats.smartMode
  });
});
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('Lau Cua 79 - Advanced Tai Xiu Prediction API v6.5 VIP');
  console.log('');
  console.log('CẢI TIẾN MỚI SIÊU CẤP VIP:');
  console.log('  - Lịch sử kiểu siêu sạch đẹp: Phiên #XXX: 🔵 XỈU → ✅ THẮNG');
  console.log('  - Thuật toán Smart Mode nâng cấp toàn diện không còn lỗi');
  console.log('  - Tự động hủy đảo (quay về chế độ normal) nếu đảo sai 3 lần liên tiếp');
  console.log('  - Tuyệt đối theo BỆT từ ngắn đến dài, khóa chức năng đảo nếu gặp bệt');
  console.log('  - Bật Học siêu tốc phân tích cực gắt nếu cắt dây đen > 2 tay');
  console.log('');
  console.log('FILE: tiendat.json, tiendat1.json');
  console.log('ID: @vilongmd5onlai');
